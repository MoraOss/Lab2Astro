...
def mostrar_histograma_y_imagen(dato_a_mostrar, titulo_histograma, titulo_imagen):
    # Genera un histograma de los valores de píxeles
    hist_values, bin_edges = np.histogram(dato_a_mostrar.flatten(), bins='auto')

    # Histograma
    plt.hist(dato_a_mostrar.flatten(), bins=bin_edges, color='b', alpha=0.7, rwidth=0.85)
    ...
    # Muestra la imagen
    plt.figure()
    plt.imshow(dato_a_mostrar, cmap='jet')
...
mostrar_histograma_y_imagen(promedio_dark, "DARK", "DARK1")
mostrar_histograma_y_imagen(promedio_bias, "BIAS", "BIAS1")
mostrar_histograma_y_imagen(promedio_flats, "FLAT", "FLAT1")
